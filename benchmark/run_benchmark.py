...
def _resolve_results_dir(cfg_default, results_dir_arg, mock, force):
    ...
    # Resolve rd the SAME way the later os.makedirs(cfg["results_dir"]) / file writes do —
    # against the CURRENT WORKING DIR, not HERE. Otherwise a command run from the repo root
    # (`--results-dir benchmark/results/matrix`) resolves to a nonexistent benchmark/benchmark/…
    # under HERE and slips the check while actually writing into the published tree.
    rd_real = os.path.realpath(rd)
    # refuse mock writes to the published results/ tree — the dir ITSELF or ANY subdir under it
    # (e.g. results/matrix would clobber the committed matrix report).
