...
def build_prompt(fixture_dir, digest, history, user_text, max_chars):
    plan = _read(os.path.join(fixture_dir, "study_plan.md"))
    progress_path = os.path.join(fixture_dir, "study_progress.initial.md")
    progress = _read(progress_path) if os.path.isfile(progress_path) else ""
...
def render_log(spec, exchanges):
    ...
    for i, (turn, reply) in enumerate(exchanges, 1):
        lines += ["## Turn %d" % i]
        if turn.get("kind"):
