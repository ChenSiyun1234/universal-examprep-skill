...
| 语言首问合并 + 派发（A8b） | 🟡（tests/test_study_state.py 语言归一化 + tests/test_en_mode_shapes.py 锚点形态 + tests/test_language_policy.py 派发钉字） | ❌ | 🟡（mock `language_first_ask`：首问三语行；set 三旗标乱序也认、非 canonical 被抓；紧迫默认档 + 绝不推断 双语） | ❌ | ❌ | en 输出的长程一致性未测 | drift 场景加 en 变体（后续项，独立于已落地的 A8c en 入口面） |
