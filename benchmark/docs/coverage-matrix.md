...
> **B2 已接通**：`behavior_smoke --llm` 单轮真 agent 冒烟不再是 skeleton——逐场景驱动 `claude -p`（或 `--agent-cmd` stub），对回复套用与 `--mock` **相同的探测器函数**（每场景的主正例检查——mock 的额外反例/变体断言用于校验探测器、不在 live 重跑）、写 transcript、输出 metrics（状态/文件类场景一次性 `-p` 不可验、诚实 SKIP）；opt-in、默认关闭、不进 CI，接线由 `tests/test_behavior_smoke_live.py` 用 stub agent 确定性验证。矩阵各行「真 LLM …未验（opt-in）」备注即指该路径。
>
...
| 学习模式 × 时间宽裕度（A6） | ✅（`update_progress` set/归一/旧四模式迁移/未知值保留 单测；紧迫别名归一；显式 `no_questions` 封顶） | 🟡（state schema：mode/time_budget/preferences） | 🟡（mock `time_budget_no_questions`：普通≤1天的真实题库 checkpoint 放行，澄清/偏好/反思式追问被抓；显式 `no_questions` 时零互动题） | ❌ | 🟡（replay `mode_urgent_no_questions`：该显式 no-questions 场景要求 `urgent_mode_questions`=0 + `urgent_mode_persisted`=1） | 真 LLM 分档节奏未验（opt-in） | 两组 scripted 会话：普通≤1天允许题库 checkpoint；显式 no-questions 零互动题 + 落盘 |
| 语言首问合并 + 派发（A8b） | 🟡（tests/test_study_state.py 语言归一化 + tests/test_language_purity.py 双语组合 + en 词表/纯度 + tests/test_language_policy.py 派发钉字） | ❌ | 🟡（mock `language_first_ask`：首问三语行；set 三旗标乱序也认、非 canonical 被抓；紧迫默认档 + 绝不推断 双语） | ❌ | ❌ | en 输出的长程一致性未测 | drift 场景加 en 变体（后续项，独立于已落地的 A8c en 入口面） |
| 小抄四段版式（阶段6 C1：必背→例题→例题解答→要点解释，例题走 visual-first） | 🟡（tests/test_language_policy.py 栏目钉 + 控制层栏目钉 + localization 钉） | ❌ | ❌ | ❌ | ❌ | 静态契约，无输出行为探测 | 低优先：若日后加 cheatsheet 冒烟场景再登记 |
...
| 难度评分 + 难题选取（A7） | ✅（`score_difficulty` 各信号维度[跨知识点/结构/需读图/多页/章节位置/题型] + 原子回写/幂等/dry-run/拒符号链接 单测；`select_hard_questions` 分类[错题/窗口→weak，在窗口/已实测→mastered]、按模式排序[查缺补漏 weak先易后难→mastered先难；零基础全局先易后难]、CLI 单测——36 例） | 🟡（`difficulty` 1-5 + `difficulty_reason` 为可选 schema 字段） | —（确定性脚本工具，非会话契约，无 mock 探测器） | ❌ | —（不产生长会话行为） | 诚实口径：评分是结构信号的**启发式下界**，非语义判定、非 per-student（用户 错题/疑难 只在 select 层加权，不写进共享题库）；LLM 语义精评为 opt-in 未来项 | 维持现状（脚本单测已覆盖各信号维度与排序契约） |
...
| 笔记本化落盘回执（v4 §2.4 先落盘再摘要） | ❌ | — | 🟡（mock `notebook_persist_ok`：教学回合须同时含 `notebook.py add-entry` 命令与学生可见 `notebook/chNN.md#` 锚点回执（章号一致）；纯聊天讲授零落盘被抓） | ❌ | ❌ | 真 LLM 落盘行为未验（opt-in）；真实 notebook/ 文件写入属 state 维度、待 drift 场景 | scripted 教学回合 → 断言回执命令 + 锚点链接 |
| 建区必确认（v4 §2.5 工作区落点） | ❌ | — | 🟡（mock `workspace_confirm_ok`：任何创建调用（`ingest.py --output-dir`/`workspace-register`）前须有落点确认问句 + 学生肯定答复；静默建区/先建后追认/拒绝后仍建 全被抓） | ❌ | ❌ | 真 LLM 首次引导流未验（opt-in） | scripted 建库请求 → 断言先问后建 |
| 额度友好产物路由（`chat` / `visual`） | ❌ | — | 🟡（mock `artifact_mode_routing`：chat 不自动渲染、显式 visual 才持久、单次 PDF 不改状态、订阅名称不触发切换、native/browser 先路由后预检） | ❌ | ❌ | 真 Agent 的宿主原生 PDF 调用仍需各运行时实测；确定性层验证命令轨迹与反例 | scripted 三种状态轨迹 + 订阅猜测反例 |
...
- **Tier 2 现有确定性 mock 层（🟡）**：`scenarios.json` 共 **17 个行为场景**，其中 **16 个在确定性 `--mock` 里被断言、进 CI、零成本**，1 个（`lazy_load_best_effort`）标了 `best_effort:true`、确定性模式下报 `SKIP` 不断言（需 transcript/真 LLM 才能验）。16 个断言场景 = 原 7 个（quiz_bank-only / 来源标注 / 提示·跳过·错题归档 / 疑难追踪 / 断点恢复 / 无 Python 降级 / 0 基础精讲已升级为七步）+ A 线新增 5 个（`visual_first_assets` 题面图门禁 / `scope_override` 越界覆盖声明 / `teaching_template` 七步模板+来源块 / `time_budget_no_questions` ≤1天节奏与显式 no-questions 分流 / `knowledge_window_recheck` 窗口外复核）+ A8b 新增 1 个（`language_first_ask` 语言首问合并）+ v4 红线新增 2 个（`notebook_persist_ok` 教学落盘回执 / `workspace_confirm_ok` 建区必确认）+ 产物资源门禁 1 个（`artifact_mode_routing`）。清单见 [`../behavior_smoke/README.md`](../behavior_smoke/README.md)。
- **Tier 4 新增确定性 replay 层（🟡）**：[`../drift/`](../drift/) 在**多轮长会话**维度覆盖目标保持 / 计划遵守 / 编题率 / 断点恢复 / 来源标注保真 / 进度持久 / wiki 越章读，以及 A6 新增的 `mode_urgent_no_questions`（学生显式要求 no-questions 时零互动题 + 紧迫开场推断并落盘）与 B3 新增的 `window_persist`（知识点窗口长会话持久化：`window_rows_added`/`window_rows_lost`）；回放脚本化 transcript + 快照，进根级测试、零成本。
