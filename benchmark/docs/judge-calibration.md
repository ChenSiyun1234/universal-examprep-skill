...
| `-2^0.5`（负底数小数指数 = 复数） | —— | 拒 None、不崩（不再 TypeError） ✓ |
| `(-2)^2`（括号负底数乘方） | 落到裸指数 `2` | 算成 `4` ✓ |
| `O(n^2)` / `O(2^n)`（复杂度记号） | 指数/底数当答案 | 跳过，不当数 ✓ |
| `答案是 42，见第 7 章 / page 12-13 / page 3/4 / equation (7)`（引用，含区间/分数/括号形） | 抓成引用数 → 判错/假阳 | 引用整段跳过，取 `42` ✓ |

`gold_answer` 也做同样归一（去千分位逗号）。取**最后一个**数字（最终答案通常在末尾）。坏 gold（非数字）
不崩、判 `(False, None)`。**已知局限**（宁拒不猜）：日期不是标量（`2020-01-01` 取末段 1，判不对 gold=2020，
金标别出日期题）；数值金标别用章节号当答案（引用跳过会误伤）。覆盖见 `tests/test_numeric_extraction.py`。

配套两条判分语义加固：**对冲不算弃答**——数值题答案里报了具体数字还挂「不确定」，按已作答记
（瞎编不能靠一句 not sure 洗成弃答）；但弃答里的**顺带计数**豁免——「材料未涵盖，我查了全部 20 讲」的
`20 讲` 是材料规模陈述、不是报答案，弃答仍成立；豁免要**双条件**：数字后紧跟量词（讲/章/页/年/lectures…）**且**数字前有材料范围提示词（材料/全部/共/查了/checked/all…）——「not sure, maybe 5 years」这类**带单位的猜测**没有提示词，仍算已作答（对冲不豁免）。
...
answers/scores 同 key **重复行**拒（人标的答案可能不是裁判判的那条）；`human_correct` 填了非 0/1
