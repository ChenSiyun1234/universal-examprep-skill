...
Tier 4 是**多轮长会话**下的漂移测量（下节），昂贵且需 LLM 裁判。Tier 2 是 Tier 4 的便宜前哨。**Tier 2 的确定性层已由本 PR（T2）落地**（真 LLM 行为仍 opt-in、不进 CI）；**Tier 4 仍未实现**（后续 PR T4）。
...
> 现状：**Tier 0–1** 低成本结构校验 + **Tier 2 行为冒烟的确定性 mock 层**（本 PR T2）均已落地进 CI；
> **Tier 2 的真 LLM 行为验证仍 opt-in、不进 CI（尚未实现）**，**Tier 4 长程漂移**仍是后续 **PR T4**。
