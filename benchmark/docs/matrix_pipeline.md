...
| 判分 | `benchmark/judge.py` / `benchmark/rejudge.py` | 数值精确比对 + lexical exact-match + 弃答；可选 LLM 复判（缓存）。`rejudge.py` 内含 `aggregate()` 单元格逻辑，但与判分/读 `results/` 耦合。 |
...
gen.py（补/生成答案，真实模型，付费）
   ...
   → report_matrix.py --summary <summary.json>（渲染）
```
