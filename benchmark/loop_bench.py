...
SKILL_PREAMBLE = ("请严格按照已安装的「期末极速备考」技能包工作（技能定义文件：%s——先读它，"
                  ...
                  "所有应落盘的产物（notebook/、mistakes/、cheatsheet.md 与 cheatsheet.pdf）"
                  "都必须真实写入本工作区。")
...
def build_sessions(course, items):
    ...
    s2 = [{"kind": "recall_mistakes", "user": S2_USER_MISTAKES},
          {"kind": "recall_content", "idx": 0, "qid": course["questions"][0],
           "user": S2_USER_CONTENT % q(course["questions"][0])}]
...
def make_preamble(cfg, course, arm):
    ...
    return SKILL_PREAMBLE % cfg["skill_md"]
