...
**测试设置**：MIT **6.006 算法**（一门很硬的理工课），65 题 = 55 道答题 + 10 道「越界探针」（材料根本没讲、考是否老实弃答）。三臂对比：
- **闭卷**（不给材料） · **给全材料**（把整份讲义 dump 进上下文） · **skill**（本技能的章节化惰性检索）

判分模型 Sonnet 4.6。下面是三模型 × 三臂的结果（百分比）。

**✅ 正确率（越高越好）—— skill 在每个模型上都最高，且都打败「闭卷」和「dump 全材料」**
...
| 模型 | 幻觉·闭卷 | 幻觉·全材料 | 幻觉·**skill** | 弃答·闭卷 | 弃答·全材料 | 弃答·**skill** |
|---|:--:|:--:|:--:|:--:|:--:|:--:|
| Opus 4.8 | 58% | 72% | **52%** | 60% | 10% | **100%** |
| Sonnet 4.6 | 62% | 80% | **52%** | 50% | 0% | **100%** |
| Haiku 4.5 | 57% | 97% | **52%** | 60% | 0% | **100%** |

![越界弃答率：材料没覆盖时是否老实说"未涵盖"](benchmark/results/matrix/chart_oos_zh.svg)

**📈 多轮迭代（skill 臂，知识库 7→14→20 章）：正确率 15% → 33% → 46%**

![迭代逼近：知识库越全越答得对](benchmark/results/matrix/chart_convergence_zh.svg)

**结论：**
1. **6.006 确实难**：闭卷正确率仅 7–24%，对 grounding 是真考验（不是送分题）。
2. **skill 的章节化惰性检索全面占优**，尤其对弱模型——把整份讲义 dump 进去反而会撑爆上下文、拖垮 Haiku（4% 正确 / 97% 幻觉）。
3. **越界探针上 skill 零瞎编**：三模型全部 100% 老实弃答；而「给材料」反而更爱编（0–10%）。
4. 多轮反馈能把正确率单调推上去。

> ⚠️ **诚实声明（Caveats）**：判分由单一模型家族（Sonnet）完成、**尚未做人工 kappa 校准**；样本 n=65 偏小；课程版权材料不入库（仅放出题面、汇总与图表，原始逐题日志本地保留）。指标对标 FACTS Grounding / Vectara HHEM / RAGAS / RGB 等权威基准（见 [`benchmark/docs/`](benchmark/docs/)）。
