...
* **📚 重点题「七步讲解模板」**：精讲重点题走固定七步：① 题面图 → ② 这题在问什么 → ③ 图里要读的量 → ④ 核心公式 → ⑤ 逐步演算 → ⑥ 答案自检 → ⑦ 知识点溯源（章节 + wiki + 可点击原文页链接），每题固定输出一行来源块 `题目来源：…｜答案来源：…｜🟢/🟡/⚠️`，收尾保留【易错点】+【3分钟速记口诀】（含文科变体）。零基础学生对老师勾的每道重点题逐题走此模板，目标是能在考场上直接默写框架拿分。
