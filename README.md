...
**测试设置**：两门名校公开课 ×（同一套题、三臂盲测、Sonnet 4.6 判分）。三臂对比：
- **闭卷**（不给材料） · **给全材料**（把整份讲义 dump 进上下文） · **skill**（本技能的章节化惰性检索）

> 🔁 **判分修订**：早期一版裁判有缺陷——会把**与标准答案一字不差**的回答（如答 `Word-RAM`、`Collision`）误判成「幻觉」，导致 skill 正确率被压成假的「都 38%」。该 bug 已修复（加确定性词法快路径 + 解析鲁棒化 + 裁判失败不再诬陷为幻觉），并对**全部答案重判**。下面是重判后的权威结果（原始 flawed 版本留存 `summary_original_judge.json` 备查）。

### 一、MIT 6.006 算法（很硬的理工课）｜65 题 = 55 答题 + 10 越界探针

**✅ 正确率（越高越好）—— skill 在每个模型上都最高，且大幅打败「闭卷」与「dump 全材料」**
...
> 这张表要**诚实地读**：幻觉率按「答案里**每一条**原子论断是否都能被讲义那一小段出处支持」严判，所以 skill 那些**正确但展开充分**的答案，常因「补充了出处段落没写的正确细节」被记为一次幻觉——这不等于编造。闭卷幻觉看着低（2–18%），是因为它**正确率也低**（只敢答一点或干脆弃答）。真正衡量「凭空捏造」的是上面的越界探针，那里 **skill = 100% 老实**。而把全讲义 dump 给弱模型会**真·崩**：Haiku 幻觉飙到 **62%**、正确率仅 2%（上下文被撑爆）。

**📈 多轮迭代（skill 臂，知识库 7→14→20 章）：正确率 28% → 62% → 87%**

![迭代逼近：知识库越全越答得对](benchmark/results/matrix/chart_convergence_zh.svg)

### 二、Yale PSYC 110 心理学（文科课）｜50 题 = 40 答题 + 10 越界探针，baseline（给材料）vs skill
...
文科课上对比更干净：skill 把正确率从 50% 拉满到 100%，越界题 100% 老实弃答（baseline 一道不弃、全瞎答）。

**结论：**
1. **6.006 确实难**：闭卷正确率仅 16–49%，对 grounding 是真考验（不是送分题）。
2. **skill 的章节化惰性检索全面占优**（85–93%），尤其对弱模型——把整份讲义 dump 进去反而撑爆上下文、拖垮 Haiku（2% 正确 / 62% 幻觉）。
3. **越界探针上 skill 零瞎编**：两门课三模型全部 100% 老实弃答；而「给材料」反而更爱编（0–10%）。
4. 多轮反馈把正确率单调推上去（28→62→87%）。

> ⚠️ **诚实声明（Caveats）**：判分仍由**单一模型家族（Sonnet）**完成、**尚未做人工 kappa 校准**（裁判会出错——这次就抓到并修了一个把正确答案误判成幻觉的 bug，更说明 kappa 校准必要）；幻觉率用的是 claim 级严判，会惩罚「正确但展开」的答案（见上）；样本偏小（6.006 n=65、PSYC n=50）；课程版权材料不入库（仅放出题面、汇总与图表，原始逐题日志本地保留）。指标对标 FACTS Grounding / Vectara HHEM / RAGAS / RGB 等权威基准（见 [`benchmark/docs/`](benchmark/docs/)）。
