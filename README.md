...
**判分如何保证可信。** 判分由 Sonnet 4.6 完成，数值题用程序精确比对。我们发现并修正了早期判分程序的一个缺陷——它会把与标准答案完全一致的回答误判为编造。修正后对全部回答重新判分，再人工抽查 16 题与判分程序对照，一致性达到 Cohen's kappa = 0.875，属于高度一致，因此下面的数字可信。
