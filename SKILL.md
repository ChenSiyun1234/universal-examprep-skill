...
* **学生可见输出按持久化 `study_state.json` `language` 单语言派发**：`中文`（缺省＝简体中文，零英文）/ `English`（零中文，用英文规范词表）/ `双语`（逐块 `中文` 前置 + `> EN:` 英文镜像、每侧各自单语言）。持久化文件与脚本输出在所有模式下保持中文规范词汇（机器词汇，不随学生语言漂移）；面向代理的控制指令保持英文、精确。完整语言策略（含单语言纯净原则与英文词表）见 [`docs/language-policy.md`](docs/language-policy.md)。
