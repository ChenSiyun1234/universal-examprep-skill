...
### 学习模式与时间宽裕度 (A6，首次对话确定并持久化)
1. **首次对话须问清两件事并存进 `study_state.json`**（`update_progress.py set --mode <模式> --time-budget <档>`）：
...
2. **≤1天 / 紧迫开场例外——严禁反而去问**：若用户开场已表明紧迫（如「明天就考」「别问我」「直接讲重点」），**不要停下来问模式/时间**——直接**推断并静默持久化**（默认 `零基础从头讲` + `≤1天`）后立即开讲。在 ≤1天 档，向用户提任何澄清/偏好问题本身就是违约（浪费复习时间）。
...
4. **重点题精讲——固定「七步讲解模板」（A5）**：精讲任何重点题（零基础速成时对老师勾的每道重点题「从零讲到会」）必须按七步顺序完整输出、不跳步不换序（详见 [`skills/exam-tutor/SKILL.md`](skills/exam-tutor/SKILL.md)）：
   ...
   * 每题在 ⑦ 后固定输出一行来源块：`题目来源：…｜答案来源：…｜<🟢/🟡/⚠️ canonical 标签>`，**默认输出到此为止**。【易错点】/【3 分钟速记】/【现在轮到你】默认不输出，仅学生主动要求或已存偏好时才给（旧版【考点拆解】/【标准答题模板/步骤】已并入 ②/④⑤，不再单列）。学习目标不变：「能在考场上默写出这道题的答题框架」。
