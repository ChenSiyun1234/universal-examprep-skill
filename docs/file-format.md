...
3. Label each displayed prompt image as `题面图 / question-side asset` and include its role/caption when available. The ALT text follows the reply-language mode: `中文` (and `双语`) sessions use the bilingual composite `题面图 / question-side asset` — that is the judging-layer surface behavior probes parse (probes only run on zh-mode transcripts); `English` sessions use `Question-side asset` (zero-CJK transcripts, screen-reader visible). Visible prose labels around the image follow the same mode (`中文` → 题面图, `English` → Question-side asset, per docs/language-policy.md).
