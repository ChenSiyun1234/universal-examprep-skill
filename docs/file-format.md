...
3. Label each displayed prompt image as `题面图 / question-side asset` and include its role/caption when available. The bilingual composite lives in the Markdown image ALT TEXT (machine/judging-layer surface — students see the rendered image, and behavior probes parse this exact form); any label the agent writes as VISIBLE prose around the image follows the reply-language mode instead (`中文` → 题面图, `English` → Question-side asset, per docs/language-policy.md).
