...
- **Difficulty × mastery selection (A7)**: the learning mode drives question ordering. When routing a practice session to `exam-quiz`, prefer the mastery-aware selector `python scripts/select_hard_questions.py --workspace <ws> --mode <学习模式> -n <k>` — it reads the bank's `difficulty` (from `scripts/score_difficulty.py`, an honest structural lower bound — never per-student, never LLM) × the student's 错题/疑难/知识点窗口 state, and orders weak-first-先易后难 (查缺补漏) or globally-先易后难 (零基础从头讲). It is deterministic heuristic ordering; the scope filter and visual-first gate still bind every item it returns.
