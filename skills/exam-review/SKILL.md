...
1. Reload mistake records from `study_progress.md`. For each recorded mistake, read its item id, fetch that exact item from `references/quiz_bank.json` by id, and have the student redo it. Replay only items already recorded; never invent or add new questions.
   - **Asset gate (fail-closed)** — the re-fetched item carries the same `requires_assets` / `assets` / `question_text_status` fields, so apply the same rule as [`exam-quiz`](../exam-quiz/SKILL.md): if `requires_assets=true`, display its `question_context`/`figure`/`diagram`/`table` assets first and **do not replay the item when a required asset is missing/unreadable or can't be shown** (e.g. web-only) — say it's blocked for lack of context and move to the next record. Treat `stub`/`page_reference` text as non-standalone: surface the asset, else the `source_file` page; if neither can be shown (no asset, original not in the workspace, web/no-image), **skip it** rather than replay an item the student can't see.
