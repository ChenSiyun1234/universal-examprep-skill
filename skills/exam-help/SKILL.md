...
### 语言 / Language
Student-facing output defaults to English (Simplified Chinese if the student opened in Chinese); a persisted `language` switches it per the dispatch rule.（学生可见输出默认英文，学生用中文开场则简体中文；工作区存了 `language`=中文/English/双语 时按派发规则切换；控制指令保持英文 / 精确。详见 [`docs/language-policy.md`](../../docs/language-policy.md)。）
